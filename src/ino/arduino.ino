...
void setup() {
    memset(frequence, 0, 5);
    memset(generator, 0, 2);
    Bridge.begin();
...
void startPython() {
  Process p;
  p.runShell("/usr/bin/python /root/acc/start.py");
}

void loop() {
    Bridge.get("g", generator, 2);
    int generatorInt = atoi(generator);
    if (generatorInt == 0) {
      freq = 0;
    } else {
      Bridge.get("f", frequence, 5);
    ...
    db = (micValue+83.2073) / 11.003;
    Bridge.put("m", db);  // fix algorithm
    // remote
    Bridge.put("r", String(digitalRead(5))); // fix this pin
}
