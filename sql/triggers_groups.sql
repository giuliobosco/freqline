...
CREATE TRIGGER freqline.insert_groups_audit
  AFTER INSERT
  ON freqline.groups
  FOR EACH ROW
  BEGIN
    ...
    INSERT INTO freqline.groups_audit (action,
                                  audit_id,
                                  audit_created_by,
                                  audit_created_date,
                                  audit_updated_by,
...
CREATE TRIGGER freqline.update_groups_audit
  AFTER UPDATE
  ON freqline.groups
  FOR EACH ROW
  BEGIN
...
CREATE TRIGGER freqline.delete_groups_audit
  AFTER DELETE
  ON freqline.groups
  FOR EACH ROW
  BEGIN
